...
import { AppMeshProxyConfiguration, AwsLogDriver, Cluster, ContainerImage, FargateService, FargateTaskDefinition, UlimitName, Protocol } from 'aws-cdk-lib/aws-ecs'
import { ApplicationLoadBalancer, ApplicationProtocol, ApplicationListener, Protocol as ELBProtocol } from 'aws-cdk-lib/aws-elasticloadbalancingv2';
import { DnsRecordType, PrivateDnsNamespace, Service } from 'aws-cdk-lib/aws-servicediscovery';
import { AccessLog, GatewayRouteSpec, HealthCheck, HttpGatewayRoutePathMatch, Mesh, RouteSpec, ServiceDiscovery, VirtualGateway, VirtualGatewayListener, VirtualNode, VirtualNodeListener, VirtualRouter, VirtualRouterListener, VirtualService, VirtualServiceProvider } from 'aws-cdk-lib/aws-appmesh'
import { CfnOutput, Duration, Stack, StackProps, Aws } from 'aws-cdk-lib'
...
export class AppMeshStack extends Stack {
  constructor(scope: Construct, id: string, props?: StackProps) {
    ...
    const vpc = Vpc.fromLookup(this, 'default-vpc', { vpcId: process.env.VPC_ID });
    ...
    new CfnOutput(this, 'alb-dns-name', { value: `https://${alb.loadBalancerDnsName}` });
    ...
    listener.addCertificates(`${id}-cert`, [
      {
        certificateArn: process.env.AWS_CERT_ARN!,
      },
    ]);
    ...
    const { service: clientFGService, port: appPort, node: clientNode } = this.createService(
      'client-1',
      cluster,
      ContainerImage.fromAsset(resolve(__dirname, '..', '..', 'src')),
      namespace,
      mesh,
      {
        NODE_ENV: 'PROD'
